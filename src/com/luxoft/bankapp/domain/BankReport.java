...
public abstract class BankReport {
    ...
    public static SortedSet<? extends Account> getAccountsSortedBySum(Bank bank) {
        return bank.getClients().stream()
                .flatMap(client -> client.getAccounts().stream())
    ...
    public static Map<Client, Collection<? extends Account>> getCustomerAccounts(Bank bank) {
        return bank.getClients().stream()
